...
int main(void)
{
  ...
  TxHeader.DLC = 8; // Length of data to send
  TxHeader.ExtId = 0;
  TxHeader.IDE = CAN_ID_STD;             // Message identifier, standard or extended
  TxHeader.RTR = CAN_RTR_DATA;           // Sending data
  TxHeader.StdId = 0x100;                // Id of this device, can be 11 bits wide
  TxHeader.TransmitGlobalTime = DISABLE; // no ideao

  testStuct.myint = 0x0069;
  testStuct.thisFloat = 45.123783F;
  testStuct.myLargerInt = 0x4206;
  testStuct.extraFloat = 45.123783F;
  testStuct.myLargestInt = 0x01234567;
  /* USER CODE END 2 */
